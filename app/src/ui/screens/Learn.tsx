...
export default function Learn() {
  ...
  return (
    <div className="page">
      ...
      <button className="mode press" disabled={busy} onClick={() => start('today')}>
        <b>今日の学習</b><span>いま最も得点につながる問題を自動で選びます</span>
      </button>
      <button className="mode press" disabled={busy} onClick={() => start('weak')}>
        <b>弱点克服</b><span>正答率の低いテーマと、まちがえた問題を集中して</span>
      </button>
      <button className="mode press" disabled={busy} onClick={() => start('quick5')}>
        <b>5分だけ復習</b><span>忘れかけと間違いを5問だけ</span>
      </button>
      <button className="mode press" disabled={busy} onClick={() => start('final')}>
      ...
      <Card>
        ...
        <button className="btn secondary press" disabled={busy} onClick={() => start('free', { free: { domain: domain || undefined }, title: domain ? `${DOMAIN_NAME[domain]}の演習` : '自由演習' })}>10問はじめる</button>
        ...
        <button className="btn ghost press" disabled={busy || !exam} onClick={() => start('free', { free: { examId: exam }, title: EXAM_OPTIONS.find((o) => o.id === exam)?.name })}>この年度を解く</button>
      </Card>

      <button className="btn ghost press" disabled={busy} onClick={() => start('diagnostic')}>初回診断をやり直す</button>
    </div>
  );
}
